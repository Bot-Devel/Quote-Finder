...
headers = {"User-Agent": "Quote-Finder-Bot/RogueOne"}
    ...
    def get_fic_metadata(self, url: str):
        ...
        try:
            ...
            for file_format in self.file_format:
                self.files[
                    self.response["urls"]["epub"]
                    .split("/")[4]
                    .split("?")[0]
                    .replace(".epub", file_format)
                ] = {
                    "hash": self.cache_hash[file_format.replace(".", "")],
                    "download_url": "https://fichub.net"
                    + cache_urls[file_format.replace(".", "")],
                }
        # Error: 'epub_url'
        # Reason: Unsupported URL
            ...
            logger.error(f"Skipping unsupported URL: {url}")
