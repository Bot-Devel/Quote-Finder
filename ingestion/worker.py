...
class JobWorker:
    ...
    async def recover_stale_jobs(self):
        async with self.session_maker() as session:
            ...
            stmt = (
                update(Job)
                .where(
                    and_(Job.status == "running", Job.last_heartbeat < stale_threshold)
                )
                .values(
                    status="failed",
        ...
        async with self.session_maker() as session:
            ...
            stmt = select(Fic).where(
                and_(
                    ...
                    or_(Fic.next_refresh_at <= now, Fic.next_refresh_at == None),
                )
            )
    ...
    async def create_refresh_job_if_needed(self, fic_id: str, source_url: str):
        async with self.session_maker() as session:
            ...
            if fic:
                fic.next_refresh_at = now + timedelta(
                    hours=fic.refresh_interval_hours or 24
                )
            await session.commit()
    ...
    async def process_job_by_id(self, job_id: str):
        ...
        async with self.session_maker() as session:
            ...
            if not row:
                logger.info(
                    f"Job {job_id} could not be claimed or is already processing."
                )
                return False
        ...
        try:
            async with self.session_maker() as repo_session:
                ...
                if job_type == "refresh":
                    res = await service.refresh_fic(
                        target_url, guild_id, update_progress
                    )
                else:
                    res = await service.ingest_fic(
                        target_url, guild_id, update_progress
                    )
