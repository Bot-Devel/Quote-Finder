...
class QdrantStore(VectorStore):
    def __init__(
        ...
        collection_name: str = "quote_finder",
    ):
    ...
    async def upsert_chunks(
        ...
        for chunk, embedding in zip(chunks, embeddings):
            point_id = str(
                uuid.uuid5(uuid.NAMESPACE_OID, f"{version_id}_{chunk.number}")
            )
    ...
    async def delete_chapters(
        ...
        await self.client.delete(
            ...
            points_selector=Filter(
                must=[
                    FieldCondition(
                        key="version_id", match=MatchValue(value=version_id)
                    ),
                    FieldCondition(
                        key="chapter_number", match=models.MatchAny(any=chapter_numbers)
                    ),
                ]
            ),
        )
            ...
            points_selector=models.Filter(
                must=[
                    models.FieldCondition(
                        key="fic_id", match=models.MatchValue(value=fic_id)
                    )
                ]
            ),
        )
