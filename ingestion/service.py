...
class IngestionService:
    ...
    async def ingest_fic(
        self, source_url: str, guild_id: int, progress_callback=None
    ) -> None:
        ...
        # 2. Sync Fic & Guild
        fic = await self.repo.get_or_create_fic(
            source_story_id, title, author, source_url
        )
        await self.repo.associate_guild(fic.id, guild_id)

        # 3. Create Version
        version = await self.repo.create_version(
            fic.id, epub_hash, self.embedding.model_name, self.embedding.dimensions
        )
        ...
        try:
            ...
            def _build_chunks():
                chunks_out = []
                for ch in chapters:
                    chs = self.chunk_builder.build_chunks(
                        ch.number, ch.paragraphs, (ch.number * 10000) + 1
                    )
            ...
            # 8. Embedding
            await report(
                f"Generating vectors for {len(all_chunks)} chunks (This will take a while)..."
            )
            chunk_texts = [c.text for c in all_chunks]

            async def embed_progress(current, total):
                await report(
                    f"Generating vectors: {current}/{total} chunks embedded..."
                )
            ...
            await self.vector_store.upsert_chunks(
                fic.id, version.id, all_chunks, embeddings
            )
    ...
    async def refresh_fic(
        ...
        if (
            getattr(self.fichub, "exit_status", 0) == 1
            or "hashes" not in self.fichub.response
        ):
            raise ValueError(
                f"FicHub returned no hashes for {source_url} — URL may be unsupported or not yet cached"
            )
        ...
        fic = await self.repo.get_or_create_fic(
            source_story_id, meta["title"], meta["author"], source_url
        )
        await self.repo.associate_guild(fic.id, guild_id)
        ...
        try:
            ...
            for ch in chapters:
                if (
                    ch.number not in old_hashes
                    or old_hashes[ch.number] != ch.chapter_hash
                ):
            ...
            await report(
                f"Detected {len(changed_chapters)} changed/new chapters. Generating chunks..."
            )
            ...
            if all_chunks:
                ...
                await self.vector_store.upsert_chunks(
                    fic.id, fic.active_version_id, all_chunks, embeddings
                )

                await report("Saving new chunks to database...")
                await self.repo.save_parsed_data(
                    fic.active_version_id, fic.id, changed_chapters, all_chunks
                )
