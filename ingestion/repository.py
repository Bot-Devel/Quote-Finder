...
class FicRepository:
    ...
    async def get_or_create_fic(
        self, source_story_id: str, title: str, author: str, source_url: str
    ) -> Fic:
    ...
    async def associate_guild(self, fic_id: str, guild_id: int) -> None:
        stmt = select(FicGuild).where(
            FicGuild.fic_id == fic_id, FicGuild.guild_id == guild_id
        )
        result = await self.session.execute(stmt)
    ...
    async def get_chapter_hashes(self, version_id: str) -> dict[int, str]:
        stmt = select(Chapter.chapter_number, Chapter.chapter_hash).where(
    ...
    async def delete_chapters(
        self, version_id: str, chapter_numbers: list[int]
    ) -> None:
    ...
    async def update_version_stats(self, version_id: str) -> None:
        ...
        chapter_count = await self.session.scalar(
            select(func.count(Chapter.id)).where(Chapter.version_id == version_id)
        )
        paragraph_count = await self.session.scalar(
            select(func.count(Paragraph.id)).where(Paragraph.version_id == version_id)
        )
