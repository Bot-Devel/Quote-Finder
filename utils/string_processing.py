...
def pos_chapter_processing(chap1):
...
def bl_chapter_processing(chap1):
    """Process the chapter heading and return chapter title and chapter url """
    ...
    url = "\nhttps://www.fanfiction.net/s/12125300/" + \
        chapter_number

    return chap1, url


def get_raw_string(type_of_search, string_to_search, book):
    ...
    if type_of_search == 1:  # searching for pos quotes
        ...
        elif book == 2:  # searching for bl quotes
            if string_to_search[0] == "'":
                if loc_of_space is not None:
                    string_processing = [r'^[']
                    string_processing.append(first_word.lower())
                    string_processing.append(r']+')
