...
class Fic(Base):
    ...
    active_version_id: Mapped[Optional[str]] = mapped_column(
        String(32), ForeignKey("fic_versions.id", use_alter=True)
    )
    ...
    # Relationships
    versions: Mapped[List["FicVersion"]] = relationship(
        "FicVersion", back_populates="fic", foreign_keys="[FicVersion.fic_id]"
    )
...
class FicVersion(Base):
    ...
    id: Mapped[str] = mapped_column(String(32), primary_key=True)
    fic_id: Mapped[str] = mapped_column(
        String(32), ForeignKey("fics.id", ondelete="CASCADE"), index=True
    )

    epub_hash: Mapped[str] = mapped_column(String(64))
    status: Mapped[str] = mapped_column(
        String(50)
    )  # e.g., building, validating, ready, active, failed, archived
    ...
    fic: Mapped["Fic"] = relationship(
        "Fic", back_populates="versions", foreign_keys=[fic_id]
    )
    chapters: Mapped[List["Chapter"]] = relationship(
        "Chapter", back_populates="version", cascade="all, delete-orphan"
    )
...
class FicGuild(Base):
    ...
    fic_id: Mapped[str] = mapped_column(
        String(32), ForeignKey("fics.id", ondelete="CASCADE"), primary_key=True
    )
