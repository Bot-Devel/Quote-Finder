...
class SearchResultView(discord.ui.LayoutView):
    def __init__(self, session, bot, renderer, service, store, fic_source_url, fic_title, query):
    ...
    async def _fetch_window(self, start_idx: int, end_idx: int):
        try:
            ref_map = {i: self.session.result_refs[i] for i in range(start_idx, end_idx) if i not in self.session.page_cache}
            if ref_map:
    ...
    async def _ensure_window_loading(self, start_idx: int, end_idx: int) -> asyncio.Task:
        window_key = (start_idx, end_idx)
        async with self.session.page_lock:
    ...
    def _trigger_prefetch(self, index: int):
        ...
        if index >= window_end - self.PREFETCH_DISTANCE:
            ...
            if next_start < len(self.session.result_refs):
                next_end = min(next_start + self.WINDOW_SIZE, len(self.session.result_refs))
    ...
    def rebuild(self):
        ...
        btn_first = discord.ui.Button(label="First", style=discord.ButtonStyle.secondary, disabled=self.session.current_index == 0)
        btn_first.callback = self._on_first
        nav_row.add_item(btn_first)
        
        btn_prev = discord.ui.Button(label="Previous", style=discord.ButtonStyle.primary, disabled=self.session.current_index == 0)
        btn_prev.callback = self._on_prev
        nav_row.add_item(btn_prev)
        
        page_indicator = discord.ui.Button(label=f"{self.session.current_index + 1} / {len(self.session.result_refs)}", style=discord.ButtonStyle.secondary, disabled=True)
        nav_row.add_item(page_indicator)
        
        btn_next = discord.ui.Button(label="Next", style=discord.ButtonStyle.primary, disabled=self.session.current_index == len(self.session.result_refs) - 1)
        btn_next.callback = self._on_next
        nav_row.add_item(btn_next)
        
        btn_last = discord.ui.Button(label="Last", style=discord.ButtonStyle.secondary, disabled=self.session.current_index == len(self.session.result_refs) - 1)
        ...
        if res:
            ...
            if url:
                btn_open = discord.ui.Button(label="Open Chapter", style=discord.ButtonStyle.link, url=url)
    ...
    async def _on_next(self, interaction: discord.Interaction):
        await self._update_page(interaction, min(len(self.session.result_refs) - 1, self.session.current_index + 1))
