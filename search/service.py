...
class SearchService:
    ...
    async def search_exact_ids(
        self, fic_id: str, version_id: str, query: str, limit: int = 10000
    ) -> tuple[list, int, bool]:
        start_time = time.time()
        query_id = uuid.uuid4().hex
        logger.info(
            f"[{query_id}] Starting EXACT search | fic={fic_id} | query='{query}'"
        )

        from ui.models import SearchResultRef

        normalized_query = self._normalize_query(query)
        line_ids, total = await self.repo.search_exact_ids(
            fic_id, version_id, normalized_query, limit + 1
        )
        ...
        refs = [
            SearchResultRef(result_id=str(i), line_id=lid)
            for i, lid in enumerate(line_ids)
        ]

        duration = time.time() - start_time
        logger.info(
            f"[{query_id}] EXACT search completed in {duration:.3f}s | Found: {total} | Truncated: {truncated}"
        )
        return refs, total, truncated

    async def search_fuzzy_ids(
        self,
        fic_id: str,
        version_id: str,
        query: str,
        ...
        query_id = uuid.uuid4().hex
        logger.info(
            f"[{query_id}] Starting FUZZY search | fic={fic_id} | query='{query}'"
        )
        ...
        normalized_query = self._normalize_query(query)
        candidates = await self.repo.get_fuzzy_candidates(
            fic_id, version_id, normalized_query, limit=300
        )
        ...
        for c_id, text, ch_id, p_num in candidates:
            ...
            if score >= min_score:
                scored.append(
                    {"id": c_id, "score": score, "ch_id": ch_id, "p_num": p_num}
                )
        ...
        refs = [
            SearchResultRef(result_id=str(i), line_id=s["id"], fuzzy_score=s["score"])
            for i, s in enumerate(scored)
        ]

        duration = time.time() - start_time
        logger.info(
            f"[{query_id}] FUZZY search completed in {duration:.3f}s | Found: {total} | Truncated: {truncated}"
        )
        return refs, total, truncated

    async def fetch_results_context(
        self, fic_id: str, version_id: str, search_type: str, ref_map: dict
    ) -> dict:
        if not ref_map:
            return {}
    ...
    async def search_semantic(
        self, fic_id: str, version_id: str, query: str, limit: int = 10
    ) -> SearchResults:
        start_time = time.time()
        query_id = uuid.uuid4().hex
        logger.info(
            f"[{query_id}] Starting SEMANTIC search | fic={fic_id} | query='{query}'"
        )
        ...
        if config.SEMANTIC_EXPANSION_ENABLED:
            if "Regulus destroying the horcrux" in query:
                queries.extend(
                    [
                        "Regulus breaks the locket",
                        "Regulus attacks the locket in the cave",
                    ]
                )
            elif "Harry was disowned" in query:
                queries.extend(
                    [
                        "Harry is disowned by the Potter family",
                        "Harry no longer considers James his father",
                        "Harry stops being a Potter",
                    ]
                )

        # Use configurable K and threshold
        retrieve_limit = (
            config.SEMANTIC_RETRIEVAL_K if config.SEMANTIC_RERANK_ENABLED else 20
        )
        ...
        for idx, q in enumerate(queries):
            ...
            dense_hits = await self.vector_store.search(
                query_vector, fic_id, version_id, limit=retrieve_limit
            )
        ...
        if config.SEMANTIC_HYBRID_ENABLED:
            ...
            for idx, q in enumerate(queries):
                prov_label = (
                    "original_lexical" if idx == 0 else f"expansion_{idx}_lexical"
                )
                lex_hits = await self.repo.get_lexical_chunks(
                    fic_id, version_id, q, limit=config.SEMANTIC_LEXICAL_K
                )
        ...
        chapters_list = await self.repo.get_chapters_by_numbers(
            fic_id, version_id, unique_ch_nums
        )
        ...
        for hit in valid_hits:
            ...
            if chapter:
                fetch_ranges.append(
                    (
                        chapter.id,
                        hit["payload"]["start_paragraph"],
                        hit["payload"]["end_paragraph"],
                    )
                )
        ...
        for hit in valid_hits:
            ...
            matched_lines = [
                p.text for p in chapter_paras if start_p <= p.paragraph_number <= end_p
            ]
        ...
        if config.SEMANTIC_RERANK_ENABLED and self.reranker and candidates:
            ...
            for c in candidates:
                ...
                for mc in merged_candidates:
                    if mc["chapter"].id == ch_id:
                        ...
                        if not (
                            c["end_p"] < mc["start_p"] - 1
                            or c["start_p"] > mc["end_p"] + 1
                        ):
                            ...
                            mlines = [
                                p.text
                                for p in ch_paras
                                if mc["start_p"] <= p.paragraph_number <= mc["end_p"]
                            ]
                            mc["chunk_text"] = "\n\n".join(mlines)

                            # Provenance
                            mc["matched_by"] = list(
                                set(mc["matched_by"] + c["matched_by"])
                            )
                            is_merged = True
                            break
            ...
            for rank, c in enumerate(candidates[:limit], start=1):
                ...
                logger.info(
                    f"[{query_id}] Rank {rank:<2} | Vector: {raw_vec:.3f} | Reranker: {rerank:>6.3f} | Prov: {prov} | Ch: {c['chapter'].chapter_number}"
                )
        else:
            limit = config.SEMANTIC_RESULT_LIMIT
            logger.info(f"[{query_id}] --- Scoring Breakdown (Top {limit}) ---")
            for rank, c in enumerate(candidates[:limit], start=1):
                ...
                prov = "+".join(c["matched_by"])
                logger.info(
                    f"[{query_id}] Rank {rank:<2} | Vector: {raw_vec:.3f} | Reranker: N/A    | Prov: {prov} | Ch: {c['chapter'].chapter_number}"
                )
        ...
        logger.info(
            f"[{query_id}] SEMANTIC search timing | Embed: {embed_duration:.3f}s | Qdrant: {qdrant_duration:.3f}s | Lexical: {lexical_duration:.3f}s | Text: {text_fetch_duration:.3f}s | Rerank: {rerank_duration:.3f}s | Render: {render_duration:.3f}s | Total: {total_duration:.3f}s"
        )
        logger.info(
            f"[{query_id}] Retreived {len(hits)} hits, reranked {len(candidates)}, returning {len(results)}."
        )
