...
class LocalFastEmbedReranker:
    def __init__(
        self,
        model_name: str = config.SEMANTIC_RERANK_MODEL,
        ...
        logger.info(
            f"Loading CrossEncoder '{model_name}'. Memory before: {mem_before:.2f} MB"
        )
        ...
        logger.info(
            f"CrossEncoder loaded. Memory after: {mem_after:.2f} MB (+{mem_after - mem_before:.2f} MB)"
        )
...
class AsyncRerankerProvider(RerankerProvider):
    def __init__(
        self, sync_provider: LocalFastEmbedReranker, executor: ThreadPoolExecutor
    ):
    ...
    async def rerank(self, query: str, documents: List[str]) -> List[float]:
        ...
        return await loop.run_in_executor(
            self._executor, self._provider.rerank_sync, query, documents
        )
