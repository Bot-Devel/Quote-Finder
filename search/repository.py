...
class QuoteSearchRepository:
    ...
    async def search_exact_ids(self, fic_id: str, version_id: str, normalized_query: str, limit: int) -> Tuple[List[str], int]:
        stmt = select(Paragraph.id).where(
    ...
    async def get_fuzzy_candidates(self, fic_id: str, version_id: str, normalized_query: str, limit: int) -> List[Tuple[str, str, str, int]]:
        stmt = select(Paragraph.id, Paragraph.normalized_text, Paragraph.chapter_id, Paragraph.paragraph_number).where(
            Paragraph.fic_id == fic_id,
            Paragraph.version_id == version_id
        ).order_by(
    ...
    async def get_chapters_by_numbers(self, fic_id: str, version_id: str, chapter_numbers: List[int]) -> List[Chapter]:
    ...
    async def fetch_contexts_bulk(self, fetch_ranges: List[Tuple[str, int, int]]) -> List[Paragraph]:
        if not fetch_ranges:
            return []
        from sqlalchemy import or_, and_
        conditions = [
            and_(Paragraph.chapter_id == cid, Paragraph.paragraph_number >= start, Paragraph.paragraph_number <= end)
            for cid, start, end in fetch_ranges
        ]
        stmt = select(Paragraph).where(or_(*conditions)).order_by(Paragraph.chapter_id, Paragraph.paragraph_number)
        result = await self.session.execute(stmt)
        return list(result.scalars().all())

    async def get_lexical_chunks(self, fic_id: str, version_id: str, query: str, limit: int) -> List[dict]:
        ...
        stmt = text("""
            ...
            LIMIT :limit
        """)
        result = await self.session.execute(stmt, {"fic_id": fic_id, "version_id": version_id, "query": query, "limit": limit})
        return [{
            "id": row.id,
