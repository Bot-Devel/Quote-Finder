...
async def focus_sentences(
    candidates, limit, query, query_id, reranker, config, fic_id, version_id
):
    """
    ...
    for c in candidates[:limit]:
        ...
        sentences = [
            s.strip() for s in re.split(r"(?<=[.!?])\s+", chunk_text) if s.strip()
        ]
    ...
    if config.SEMANTIC_RERANK_ENABLED and reranker and flat_sentences:
        ...
        logger.info(
            f"[{query_id}] Sentence focusing latency: {sentence_latency:.3f}s for {len(flat_sentences)} sentences"
        )
    else:
