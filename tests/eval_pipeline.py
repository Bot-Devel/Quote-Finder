...
EVAL_FIXTURE = [
    {
        ...
        "expected_substrings": [
            {"chapter_number": 81, "text": "charged Kreacher with destroying it", "grade": 3},
            {"chapter_number": 125, "text": "Regulus ran into the forest as well before Pettigrew could target him", "grade": 1}
        ]
    },
    {
        "query": "Harry was disowned",
        "expected_substrings": [
            {"chapter_number": 126, "text": "Harry would automatically become a Black if he were disowned as a Potter.", "grade": 3},
...
async def resolve_fixtures(session, fic_id: str, version_id: str) -> List[Dict]:
    ...
    for item in EVAL_FIXTURE:
        ...
        for label in item["expected_substrings"]:
            ...
            matching_paras = [p for p in all_paras if text_sub.lower() in p.text.lower()]
            
            if not matching_paras:
                raise ValueError(f"Failed to resolve fixture: Could not find substring '{text_sub}' in Chapter {ch_num}")
            ...
            print(f"DEBUG: Resolved '{text_sub}' to Chapter {ch_num} Paragraph {para.paragraph_number} (Matched {len(matching_paras)} paras)")
        ...
        resolved_fixtures.append({
            "query": item["query"],
            "relevance_labels": resolved_labels
        })
...
def compute_ndcg(retrieved_relevances: List[int], ideal_relevances: List[int], k: int = 10) -> float:
...
async def evaluate_pipeline(service: SearchService, fic_id: str, version_id: str, resolved_fixtures: List[Dict], actual_model: str, mem_before: float):
    k_dense = config.SEMANTIC_RETRIEVAL_K
    hybrid = config.SEMANTIC_HYBRID_ENABLED
    rerank = config.SEMANTIC_RERANK_ENABLED
    ...
    print(f"Runtime/Provider:  FastEmbed")
    print(f"=======================================================")
    
    for item in resolved_fixtures:
        query = item["query"]
        labels = item["relevance_labels"]
        target_count = len([l for l in labels if l["grade"] >= 2])
        
        start_time = time.time()
        res = await service.search_semantic(fic_id, version_id, query, limit=50) # fetch up to 50 for Recall@30
        ...
        # Build query table
        print(f"\nQuery: '{query}'")
        print(f"{'Expected Passage (Semantic Match)':<40} | {'Entered Cand. Set':<18} | {'Dense Rank':<11} | {'Final Rank':<10}")
        print("-" * 87)
        ...
        target_ranks = {l["text"]: None for l in labels if l["grade"] >= 2}
        
        for idx, c in enumerate(candidates):
            grade, matched_text = check_overlap(c["chapter"].chapter_number, c["start_p"], c["end_p"], labels)
        ...
        for label in labels:
            ...
            for idx, c in enumerate(candidates):
                grade, matched_text = check_overlap(c["chapter"].chapter_number, c["start_p"], c["end_p"], [label])
                if grade >= 2:
                    entered_set = "Yes"
            ...
            print(f"{disp_text:<40} | {entered_set:<18} | {str(dense_rank):<11} | {str(final_rank):<10}")
            
            if final_rank != "N/A":
                expected_ranks.append(final_rank)
        
        mrr = compute_mrr(retrieved_relevances)
        ideal = [l["grade"] for l in labels]
    ...
    print(f"Peak Memory:               {mem_peak:.1f}MB (+{mem_peak - mem_before:.1f}MB)")
...
async def main():
    ...
    vector_store = QdrantStore(url=os.getenv("QDRANT_URL", "http://localhost:6333"), api_key=os.getenv("QDRANT_API_KEY"))
    ...
    async with session_maker() as session:
        ...
        try:
            resolved_fixtures = await resolve_fixtures(session, fic.id, fic.active_version_id)
        ...
        await evaluate_pipeline(service, fic.id, fic.active_version_id, resolved_fixtures, actual_model, mem_before)
