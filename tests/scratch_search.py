...
async def main():
    ...
    async with session_maker() as session:
        ...
        result = await session.execute(
            select(Chapter).where(Chapter.version_id == fic.active_version_id, Chapter.chapter_number == 126)
        )
        ch_126 = result.scalars().first()
        
        # Chapter 80
        result = await session.execute(
            select(Chapter).where(Chapter.version_id == fic.active_version_id, Chapter.chapter_number == 80)
        )
        ...
        if ch_126:
            ...
            print("--- CH 126 ---")
            for p in paras:
                if "disown" in p.text.lower() or "prince of slytherin" in p.text.lower() or "black" in p.text.lower():
        ...
        if ch_80:
            ...
            print("--- CH 80 ---")
            for p in paras:
                if "regulus" in p.text.lower() or "horcrux" in p.text.lower() or "locket" in p.text.lower():
                    print(f"[{p.paragraph_number}] {p.text}")
