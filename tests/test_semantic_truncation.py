...
async def main():
    ...
    reranker = AsyncRerankerProvider(LocalFastEmbedReranker(config.SEMANTIC_RERANK_MODEL), executor)
    ...
    async with session_maker() as session:
        ...
        res = await service.search_semantic(fic.id, fic.active_version_id, query, limit=5)
        ...
        for idx, r in enumerate(res.results):
            ...
            matched_lines = [p.text for p in sorted(all_paragraphs, key=lambda p: p.paragraph_number)]
            expected_chunk_text = "\n\n".join(matched_lines)
            ...
            print("="*60)
            print(f"Rank {idx+1} | Ch: {r.chapter_number} | Score: {r.semantic_score:.3f}")
            print(f"--- Displayed Text ---")
            print(r.matched_text)
            
            assert r.matched_text == expected_chunk_text, f"Mismatch at rank {idx+1}!\nExpected:\n{expected_chunk_text}\nGot:\n{r.matched_text}"
            print(f"SUCCESS: Displayed passage exactly matches the trimmed chunk text.")
