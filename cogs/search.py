...
class SearchCog(commands.Cog):
        ...
        try:
            async with typing_indicator():
                async with self.bot.session_maker() as session:
                    ...
                    if interaction:
                        msg = await interaction.followup.send(view=view, allowed_mentions=discord.AllowedMentions.none(), wait=True)
                    else:
                        msg = await ctx.reply(view=view, allowed_mentions=discord.AllowedMentions.none(), mention_author=False)
    ...
    async def qf_base(self, ctx, *, query: Optional[str] = None):
        embed = discord.Embed(title="📚 Quote Finder Help", color=discord.Color.blurple())
