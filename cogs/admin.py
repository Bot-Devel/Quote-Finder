...
class StatusPaginationView(discord.ui.View):
    def __init__(self, embeds, fics_chunks, cog, ctx):
        ...
        if ctx.author.id == config.ROOT_USER_ID:
            self.refresh_select = discord.ui.Select(
                placeholder="Select a fic to Delta Refresh...", row=0
            )
            self.refresh_select.callback = self._on_refresh
            self.rebuild_select = discord.ui.Select(
                placeholder="Select a fic to Full Rebuild...", row=1
            )
    ...
    async def _on_refresh(self, interaction: discord.Interaction):
        if interaction.user.id != config.ROOT_USER_ID:
            await interaction.response.send_message(
                f"❌ You don't have permission to do this! Please ping <@{config.ROOT_USER_ID}> if you need a fic updated.",
                ephemeral=True,
            )
            return
        ...
        self.cog.bot.loop.create_task(
            self.cog.refresh(self.ctx, target=fic_id, interaction=interaction)
        )
    ...
    async def _on_rebuild(self, interaction: discord.Interaction):
        ...
        self.cog.bot.loop.create_task(
            self.cog.rebuild(self.ctx, target=fic_id, interaction=interaction)
        )
...
class Admin(commands.Cog):
    ...
    async def _process_job_background(self, message, job_id, prefix_text):
        ...
        tracker_task = self.bot.loop.create_task(
            self._track_job(message, job_id, prefix_text)
        )
    ...
    async def rebuild(self, ctx, target: str, interaction: discord.Interaction = None):
        ...
        async with typing_indicator():
            async with self.bot.session_maker() as session:
                stmt = select(Fic).where(
                    or_(Fic.id == target, Fic.source_story_id == target)
                )
        ...
        self.bot.loop.create_task(
            self._process_job_background(
                msg, job_id, f"**Rebuilding {fic_title}** (Job: `{job_id[-8:]}`)"
            )
        )
    ...
    async def refresh(self, ctx, target: str, interaction: discord.Interaction = None):
        ...
        self.bot.loop.create_task(
            self._process_job_background(
                msg, job_id, f"**Delta Refreshing {fic_title}** (Job: `{job_id[-8:]}`)"
            )
        )
    ...
    async def connect(self, ctx, target: str, guild_id: int):
        ...
        async with ctx.typing():
            async with self.bot.session_maker() as session:
                ...
                stmt_link = select(FicGuild).where(
                    FicGuild.fic_id == fic.id, FicGuild.guild_id == guild_id
                )
                if (await session.execute(stmt_link)).scalar_one_or_none():
                    await ctx.send(
                        f"⚠️ Fic `{fic.title}` is already connected to server `{guild_id}`."
                    )
                    return
            ...
            await ctx.send(
                f"✅ Successfully connected Fic **{fic.title}** to server `{guild_id}`!"
            )
        ...
        async with ctx.typing():
            async with self.bot.session_maker() as session:
                ...
                current_embed = discord.Embed(
                    title="Tracked fanfictions", color=discord.Color.blue()
                )
                ...
                for fic in fics:
                    ...
                    last_fetched = (
                        fic.last_refreshed_at.strftime("%Y-%m-%d %H:%M UTC")
                        if fic.last_refreshed_at
                        else "Never"
                    )
